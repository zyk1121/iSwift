...
let kYKPOPURLString = "yk://istudydemo/thirdpart/pop"

class YKPOPViewController: UIViewController {
    ...
    static func portalLoad()
    {
        YKPortal.registerPortalWithHandler(handler: { (transferURL:NSURL, transferType:YKTransferType, sourceViewController:UIViewController) -> UIViewController? in
            ...
            if transferURL.hasSameTrunkWithURL(URL: NSURL(string:kYKPOPURLString)!) {
                let viewController = YKPOPViewController()
        ...
        }, prefixURL: NSURL(string: kYKPOPURLString)!)
    }
    ...
    func setupUI() {
        ...
        testBtn?.addTarget(self, action: #selector(YKPOPViewController.testBtnClicked(sender:)), for: UIControlEvents.touchUpInside)

...
let kYKNetworkURLString = "yk://istudydemo/network"

class YKNetworkViewController: UIViewController {
    ...
    static func portalLoad()
    {
        YKPortal.registerPortalWithHandler(handler: { (transferURL:NSURL, transferType:YKTransferType, sourceViewController:UIViewController) -> UIViewController? in
            ...
            if transferURL.hasSameTrunkWithURL(URL: NSURL(string:kYKNetworkURLString)!) {
                let viewController = YKNetworkViewController()
        ...
        }, prefixURL: NSURL(string: kYKNetworkURLString)!)
    }

//
//  YKWeiboMessageViewController.swift
//  iStudySwift
//
...
class YKWeiboMessageViewController: YKWeiboBaseTableViewController {
    ...
    static func portalLoad()
    {
        YKPortal.registerPortalWithHandler(handler: { (transferURL:NSURL, transferType:YKTransferType, sourceViewController:UIViewController) -> UIViewController? in
            ...
            if transferURL.hasSameTrunkWithURL(URL: NSURL(string:kYKWeiboMessageURLString)!) {
                let viewController = YKWeiboMessageViewController()
        ...
        }, prefixURL: NSURL(string: kYKWeiboMessageURLString)!)
    }

...
let kYKWeiboMeURLString = "yk://istudydemo/weibo/me"

class YKWeiboMeViewController: YKWeiboBaseTableViewController {
    ...
    static func portalLoad()
    {
        YKPortal.registerPortalWithHandler(handler: { (transferURL:NSURL, transferType:YKTransferType, sourceViewController:UIViewController) -> UIViewController? in
            ...
            if transferURL.hasSameTrunkWithURL(URL: NSURL(string:kYKWeiboMeURLString)!) {
                let viewController = YKWeiboMeViewController()
        ...
        }, prefixURL: NSURL(string: kYKWeiboMeURLString)!)
    }
    ...
    override func viewDidLoad() {
        ...
        setupUI()
        if !userLogin {
            visitorView!.setupVisitorInfo(isHome: false, imageName: "visitordiscover_image_profile", message: "1234567")
        }
